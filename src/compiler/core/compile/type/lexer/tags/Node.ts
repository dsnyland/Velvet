import { BaseToken, CharFunc, IdentifierToken, LexerCharMap, SimpleToken, SourcePos, StringToken, TextToken, Token, TokenKind } from "../types/Node.types";
...
export class Lexer {
  ...
  private LexerCharMap: LexerCharMap = {
    ...
    "/": () => {
      ...
      // If it's just '/', treat as text fallback later
      return this.lexText();
    },
  ...
  private nextToken(): Token {
    ...
    if (handler) {
      ...
      if (t.kind === TokenKind.TagEnd || t.kind === TokenKind.SelfCloseEnd) this.inTag = false;
      return t;
    }
  ...
  /*
    ...
    private lexTag(): Token[] {
      const tokens: Token[] = [];
      while (!this.isEOF()) {
        const c = this.peek(0);
        if (c === ">") break;
        if (this.isAlpha(c)) {
          tokens.push(this.lexIdentifier()); // prop name
          continue;
        }
        if (c === "=") {
          tokens.push(this.simple(TokenKind.PropEqual, 1));
          continue;
        }
        if (c === '"' || c === "{") {
          tokens.push(this.lexAttributeValue());
          continue;
        }
        this.advance(1);
      }
  ...
  */
  ...
  private lexAttributeValue(): Token {
    if (this.peek(0) === '"') {
      return this.lexString();
    }
    if (this.peek(0) === "{") {
  ...
  private lexJSExpression(): Token {
    ...
    while (!this.isEOF() && depth > 0) {
      ...
      if (c === "{") {
        ...
        depth--;
        if (depth === 0) {
          this.advance(1); // consume closing }
          break;
        }
      }
      if (depth > 0) {
  ...
  private simpleAt(kind: TokenKind, start: SourcePos, end: SourcePos): BaseToken {
  ...
  private isIdentPart(c: string): boolean {
    return this.isAlpha(c) || this.isDigit(c) || c === "_" || c === "-" || c === ":";
  }
}
